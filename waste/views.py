...
def view_by_name_search(request):
    ...
    location_res = Lokalizacja.objects.filter(id_odpadu=waste_res)
    ...
    isotopes_res = Izotopy.objects.filter(id_odpadu=waste_res)
    measurements = Pomiar.objects.filter(odpad_id=waste_res).order_by('-data')
